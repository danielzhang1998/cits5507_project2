...
double *call_omp_quick(double time_start, double time_end, int rank, double *mpi_new_array, int num_value_per_process, int num_of_process, int merge_length, int array_length, double *mpi_omp_quick_result_1, int num_value_left, int num_of_thread){
	...
	if (rank == 0){
		...
		save_csv("sort_result.csv", "serial_quick", array_length, result_quick, num_of_process, num_of_thread);
				...
				#pragma omp critical
				{
					if(rank % (2 * step) == 0)
					{
						...
						if (source_process < num_of_process)  //  if the num_of_process is an odd num, will not waiting for receiving other's sending
						{
							...
							MPI_Recv(&quick_length, 1, MPI_INT, source_process, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); //  receive the length of array to be quickd
							//printf("%d\n", quick_length);
							double *quick_array = (double *)malloc(quick_length * sizeof(double));

							MPI_Recv(quick_array, quick_length, MPI_DOUBLE, source_process, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); //  receive the quick to be quickd
