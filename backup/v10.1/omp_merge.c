...
double *call_omp_merge(double time_start, double time_end, int rank, double *mpi_new_array, int num_value_per_process, int num_of_process, int merge_length, int array_length, double *mpi_omp_merge_result_1, int num_value_left, int num_of_thread){
	...
	if (rank == 0){
		...
		save_csv("sort_result.csv", "mpi_openmp_merge", array_length, result_merge_mpi_omp, num_of_process, num_of_thread);
	}
	...
	if (rank == 0){
		...
		double result_merge = compare_with_merge_serial(mpi_new_array, mpi_omp_merge_result_1, mpi_omp_merge_result_1, array_length);
		save_csv("sort_result.csv", "serial merge", array_length, result_merge, num_of_process, num_of_thread);
		printf("\n=============\n");
	}

	return mpi_omp_merge_result_1;
}
