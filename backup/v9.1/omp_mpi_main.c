...
int main(int argc, char *argv[])
{
    ...
    if(strcmp(argv[2], "quick") == 0){
        ...
        receive_array = mpi_quick_main(rank, mpi_new_array, num_value_per_process, num_of_process, merge_length);
    ...
    //printf("%d\n", num_value_per_process);
        ...
        receive_array = mpi_merge_main(rank, mpi_new_array, num_value_per_process, num_of_process, merge_length);
    ...
    //printf("%d\n", num_value_per_process);
        ...
        receive_array = mpi_enum_main(rank, mpi_new_array, num_value_per_process, num_of_process, merge_length);
        if (rank == 0)
            gettimeofday(&middle, NULL);
        //printf("test1\n");
        //double *mpi_omp_enum_result = mpi_omp_enum_main(rank, mpi_new_array, num_value_per_process, receive_array, num_of_process, merge_length, num_of_thread);
        if (rank == 0)
    ...
    //printf("%d\n", num_value_per_process);
        ...
        time_start = MPI_Wtime();
        double *mpi_omp_merge_result = mpi_omp_merge_main(rank, mpi_new_array, num_value_per_process, num_of_process, merge_length, num_of_thread);         
        time_end = MPI_Wtime();
        if (rank == 0){
